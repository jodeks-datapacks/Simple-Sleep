...
open class RunHytaleExtension {
    ...
    var assetsPath: String = ""  // Path to Assets.zip
}
...
open class RunServerTask : DefaultTask() {
    ...
    fun run() {
        ...
        val assets = assetsPath.getOrElse("")
        if (assets.isNotEmpty()) {
            val assetsFile = File(assets)
            if (assetsFile.exists()) {
                javaArgs.addAll(listOf("--assets", assetsFile.absolutePath))
                println("Using assets from: ${assetsFile.absolutePath}")
            } else {
                println("WARNING: Assets path configured but file not found: $assets")
                println("Server may fail to start without assets!")
            }
        } else {
            println("WARNING: No assets path configured!")
            println("Configure assetsPath in build.gradle.kts: runHytale { assetsPath = \"path/to/Assets.zip\" }")
        }
